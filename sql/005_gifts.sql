-- =====================================================================
-- Праздники (раздел «Подарки») и тип цели «копилка подарков»
-- Выполнить один раз: Supabase → SQL Editor → New query → вставить всё → Run.
-- Скрипт добавляет таблицу gifts, разрешает у цели тип 'gifts'
-- и обновляет функции get_debts_state / save_debts_state: теперь они возят ещё и ключ gifts.
-- Предыдущие скрипты (debts.sql, calendar_update.sql, actual_expenses.sql) должны быть выполнены.
-- =====================================================================

-- ---------- тип цели: добавляем «копилку подарков» ----------
alter table public.goal_settings drop constraint if exists goal_settings_kind_check;
alter table public.goal_settings
  add constraint goal_settings_kind_check check (kind in ('bucket', 'reserve', 'gifts'));

-- ---------- праздники ----------
-- day/month — день и месяц праздника; year нужен только для разовых (repeat_kind = 'once')
...
drop policy if exists own_rows on public.gifts;
create policy own_rows on public.gifts
